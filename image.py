...
def image_field_response(image_field):
    ...
    return {"data": data_base64, "mime_type": mime_type}
...
def create_thumbnail(image_field):
    ...
    resized = im.resize((int(width / factor), int(height / factor)))
...
def save_pil_image(pil_image, filepath):
    ...
    path = default_storage.save(filepath, ContentFile(buffer.getvalue()))
    return path
